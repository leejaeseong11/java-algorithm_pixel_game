...
public class PixelPlayer06 extends Player {
	...
	int[][] myMap = new int[PixelTester.SIZE_OF_BOARD][PixelTester.SIZE_OF_BOARD]; // 나의 가중치를 저장하기 위한 맵
	int[][] enMap = new int[PixelTester.SIZE_OF_BOARD][PixelTester.SIZE_OF_BOARD]; // 상대방의 가중치를 저장하기 위한 맵
	...
	int lose_cnt_row = 0; // 둘 곳의 행에서 지는 곳의 수를 저장
	int lose_cnt_col = 0; // 둘 곳의 열에서 지는 곳의 수를 저장
	...
	Point nextPosition(Point lastPosition) {
		...
		if (isLose() == true) {
			// 만약 무조건 지는 수가 하나 있으면 가중치를 높게 설정
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (map[i][j] == order) { // 나의 돌에 따른 가중치 부여
					if (i != 0)
						myMap[i - 1][j] += 6; // 위쪽 가중치 4 부여
					if (i != 7)
						myMap[i + 1][j] += 6; // 아래쪽 가중치 4 부여
					if (j != 0)
						myMap[i][j - 1] += 6; // 왼쪽 가중치 4 부여
					if (j != 7)
						myMap[i][j + 1] += 6; // 오른쪽 가중치 4 부여
					if (i != 7 && j != 7)
						myMap[i + 1][j + 1] += 8; // 오른쪽아래 가중치 5 부여
					if (i != 0 && j != 0)
						myMap[i - 1][j - 1] += 8; // 왼쪽위 가중치 5 부여
					if (i < 6 && j < 6)
					...
					if (i > 0 && j > 1)
						myMap[i - 1][j - 2] += 3;
					if (j < 6)
						myMap[i][j + 2] += 6; // 왼쪽 오른쪽*2 가중치 부여
					if (j > 1)
						myMap[i][j - 2] += 6;
					if (i < 6)
						myMap[i + 2][j] += 6; // 위 아래*2 가중치 부여
					if (i > 1)
						myMap[i - 2][j] += 6;
				} else if (map[i][j] == enemy) { // 상대의 돌에 따른 가중치 부여
					if (i != 0)
						enMap[i - 1][j] += 7; // 위쪽 가중치 4 부여
					if (i != 7)
						enMap[i + 1][j] += 7; // 아래쪽 가중치 4 부여
					if (j != 0)
						enMap[i][j - 1] += 7; // 왼쪽 가중치 4 부여
					if (j != 7)
						enMap[i][j + 1] += 7; // 오른쪽 가중치 4 부여
					if (i != 7 && j != 7)
						enMap[i + 1][j + 1] += 9; // 오른쪽아래 가중치 5 부여
					if (i != 0 && j != 0)
						enMap[i - 1][j - 1] += 9; // 왼쪽위 가중치 5 부여
					if (i < 6 && j < 6)
						enMap[i + 2][j + 2] += 1; // 대각선*2 가중치 부여
					if (i > 1 && j > 1)
						enMap[i - 2][j - 2] += 1;
					if (i < 6 && j < 7)
						enMap[i + 2][j + 1] += 4; // 대각선*2 옆 가중치 부여
					if (i > 1 && j > 0)
						enMap[i - 2][j - 1] += 4;
					if (i < 7 && j < 6)
						enMap[i + 1][j + 2] += 4; // 대각선*2 옆 가중치 부여
					if (i > 0 && j > 1)
						enMap[i - 1][j - 2] += 4;
					if (j < 6)
						enMap[i][j + 2] += 5; // 왼쪽 오른쪽*2 가중치 부여
					if (j > 1)
						enMap[i][j - 2] += 5;
					if (i < 6)
						enMap[i + 2][j] += 5; // 위 아래*2 가중치 부여
					if (i > 1)
						enMap[i - 2][j] += 5;
				}
			}
		...
		// 나의 맵에 상대의 가중치까지 고려한 수치를 합산하여 저장
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (i > 0 && i < 7 && j > 0 && j < 7)
					myMap[i][j] += (enMap[i][j]);
				// 테두리에 있는 인덱스 라면 가중치를 -1 하며 저장
				else
					myMap[i][j] += (enMap[i][j] - 1);
			}
		}
		...
		while (check) { // 나의 다음 수가 위험한지 확인하기 위한 반복문
			...
			// 나의 다음 수가 어디에 둬도 위험한 경우
			if (best_col <= 0 && best_row <= 0) {
				// 랜덤하게 두도록 설정
				Random rand = new Random();
				while (true) {
					row_index = rand.nextInt(8);
					col_index = rand.nextInt(8);
					if (isPossible(row_index, col_index)) {
						nextPosition = new Point(row_index, col_index);
						break;
					}
				}
				break;
			...
			if (best_row >= best_col) { // 행이동에 대한 가중치값이 열이동에 대한 가중치 값보다 높을 경우
				if (OneMoreAnalysis(row_index, lastCol, "row")) { // 두는 곳이 위험한지 확인
					best_row = -1;
					best_col = -1;
					check = true;
				} else { // 위험하지 않으면 좌표 저장
	...
	boolean isLose() { // 막지 않으면 지는 경우
		...
		for (int col = 0; col < PixelTester.SIZE_OF_BOARD; col++) {
			if (isPossible(lastRow, col)) {
				...
				//     ㅇ == 3
				if (col < 6 && lastRow < 6) {
					if (map[lastRow + 1][col + 1] == enemy && map[lastRow + 2][col + 2] == enemy) { 
						// 상대의 돌이 1번과 2번에 있는 경우
						myRow = lastRow;
						myCol = col;
						cnt++;
						continue;
				...
				if (col > 0 && lastRow > 0 && lastRow < 7 && col < 7) {
					if (map[lastRow - 1][col - 1] == enemy && map[lastRow + 1][col + 1] == enemy) { 
						// 상대의 돌이 1번과 3번에 있는 경우
						myRow = lastRow;
						myCol = col;
						cnt++;
						continue;
		...
		if (cnt == 1) {// 행 고정시 위험한 곳이 한 군데이면 true
			// 만약 현재 위험한 열이 한 군대일지라도, 그 곳을 두었을 때 그 열에 대한 행 중 위험한 곳이 있다면
			// lose_cnt_col 변수를 2로 설정
			if (this.OneMoreAnalysis(myRow, myCol, "col")) {
				lose_cnt_col = 2;
			}
			return true;
		}
		...
		for (int row = 0; row < PixelTester.SIZE_OF_BOARD; row++) {
			if (isPossible(row, lastCol)) {
				...
				//     ㅇ == 3
				if (lastCol < 6 && row < 6) {
					if (map[row + 1][lastCol + 1] == enemy && map[row + 2][lastCol + 2] == enemy) { 
						// 상대의 돌이 1번과 2번에 있는 경우
						myRow = row;
						myCol = lastCol;
						cnt++;
				...
				if (lastCol > 0 && row > 0 && row < 7 && lastCol < 7) {
					if (map[row - 1][lastCol - 1] == enemy && map[row + 1][lastCol + 1] == enemy) { 
						// 상대의 돌이 1번과 3번에 있는 경우
						myRow = row;
						myCol = lastCol;
						cnt++;
		...
		if (cnt == 1) { // 위험한 곳이 한 군데이면 true
			// 만약 현재 위험한 행이 한 군대일지라도, 그 곳을 두었을 때 그 행에 대한 열 중 위험한 곳이 있다면
			// lose_cnt_row 변수를 2로 설정
			if (this.OneMoreAnalysis(myRow, myCol, "row")) {
				lose_cnt_row = 2;
			}
			return true;
		}
	...
	boolean OneMoreAnalysis(int inputRow, int inputCol, String opt) {
		...
		// 내가 이번 턴 row 중 한 곳에 두었을 경우, 그 row에 대한 모든 col에 대해 위험성 분석
		// (매개변수가 "row" 로 입력되었을 경우)
		if (opt.equals("row")) {
			for (int i = 0; i < PixelTester.SIZE_OF_BOARD; i++) {

				// 만약 row 중 한 곳을 막아야만 할 때(가중치가 제일 높을 때), 그 곳을 놓았을 때 행이나 열에 다른 위험한 수가 있다면
				// 그 곳을 막지 않고 회피하도록 가중치를 0 으로 만듦
				if (lose_cnt_row >= 2) {
		...
		// 내가 이번 턴 col 중 한 곳에 두었을 경우, 그 col에 대한 모든 row에 대해 위험성 분석
		// (매개변수가 "row" 가 아닌 다른 것으로 입력되었을 경우)
		else {
			for (int i = 0; i < PixelTester.SIZE_OF_BOARD; i++) {
				// 해당 위치에 돌을 놓은 후 고려하는 조건부 검색 이기 때문에, 놓을 곳은 검색 제외
				if (i == inputRow)
				...
				// 만약 col 중 한 곳을 막아야만 할 때(가중치가 제일 높을 때), 그 곳을 놓았을 때 행이나 열에 다른 위험한 수가 있다면
				// 그 곳을 막지 않고 회피하도록 가중치를 0 으로 만듦
				if (lose_cnt_col >= 2) {
					for (int j = 0; j < PixelTester.SIZE_OF_BOARD; j++) {
						if (isPossible(inputRow, j)) {
